...
3. Użyj instrukcji warunkowej if z in do sprawdzenia czy liczba 3 jest w liście data,
    pokaż informację w konsoli jeśli zachodzi taka sytuacja. Przykład:
        if 100 in someList:
            print("100 jest w liście")
4. Użyj pętli for aby pokazać wartości w liście
    for el in someList:
        print("element listy z dodaną wartością 2", el + 2)
5. Użyj pętli for aby przejść po elementach listy, ale pokaż ich wartości pomnożone przez 2
"""
